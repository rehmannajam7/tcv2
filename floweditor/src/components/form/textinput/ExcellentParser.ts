...
const isValidStart = (
  ...
  if (body[0] === '(') {
    ...
    // Empty body is valid when allowing incomplete expressions (user is just starting to type)
    if (allowIncomplete && topLevel === '') {
      return true;
    }
...
export default class ExcellentParser {
  ...
  public expressionContext(textToCaret: string): string {
    ...
    const body = lastExpression.text.substring(1); // return without prefix

    // Return null for empty body (standalone @ symbol) to allow user to continue typing
    if (body === '') {
      return null;
    }
    ...
    for (let pos = 0; pos < text.length; pos++) {
      ...
      if (state === STATE_BODY) {
        if (
          ...
          (isWordChar(nextCh) || nextCh === '(' || nextCh === 0)
        ) {
          state = STATE_PREFIX;
          currentExpression = {
            start: pos,
      ...
      if (
        ...
        } else if (state === STATE_PREFIX && allowIncomplete) {
          // Handle case where user typed @ but hasn't typed anything after it yet
          currentExpression.closed = false;
          currentExpression.end = pos + 1;
          expressions.push(currentExpression);
        }
