...
> {
  ...
  private handleChannelUpdate(selection: Asset[], submitting = false): boolean {
    const chan = selection && selection.length ? selection[0] : null;
    return this.handleUpdate({ channel: chan }, submitting);
  }

  private handleLanguageUpdate(selection: any[], submitting = false): boolean {
    const lang = selection && selection.length ? selection[0] : null;
    return this.handleUpdate({ language: lang }, submitting);
  }
  ...
  private onUpdated(dispatch: DispatchWithState, getState: GetState): void {
    ...
    if (this.state.field && this.state.field.value && this.state.field.value.type === AssetType.Field) {
      dispatch(
        updateAssets(
      ...
      return (
        <TextInputElement
          ...
          placeholder={i18n.t('forms.enter_field_value', {
            field: getName(this.state.field.value || {}),
          })}
