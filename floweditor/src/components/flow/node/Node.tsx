...
export class NodeComp extends React.PureComponent<NodeProps> {
  ...
  constructor(props: NodeProps, context: any) {
    ...
    console.log('NodeComp constructor mutable debug:', context?.config?.mutable);
  ...
  private handleAddToNode(): void {
    console.log(
      '🔵 Node.handleAddToNode: called for node:',
      this.props.renderNode.node,
    );
    console.log(
      '🔵 Node.handleAddToNode: onAddToNode prop:',
      this.props.onAddToNode,
    );

    if (this.props.onAddToNode) {
      console.log(
        '🔵 Node.handleAddToNode: calling onAddToNode with node:',
        this.props.renderNode.node,
      );
      this.props.onAddToNode(this.props.renderNode.node);
      console.log('🔵 Node.handleAddToNode: onAddToNode called successfully');
    } else {
      console.error('🔴 Node.handleAddToNode: onAddToNode prop is missing!');
    }
  }
  ...
  private onClick(event: React.MouseEvent<HTMLElement>): void {
    console.log('NodeComp onClick debug: mouseState=', this.props.mouseState, 'mutable=', this.context?.config?.mutable);
  ...
  public render(): JSX.Element {
    ...
    if (type !== Types.execute_actions) {
      ...
      if (
        ...
        console.log(
          '🟢 Node.render: Creating add button for node:',
          this.props.renderNode.node.uuid,
        );
        console.log('🟢 Node.render: translating:', this.props.translating);
        console.log(
          '🟢 Node.render: config.mutable:',
          this.context.config.mutable,
        );
        console.log('🟢 Node.render: hasCallBrainAction:', hasCallBrainAction);
        ...
        console.log(
          '🔴 Node.render: NOT creating add button for node:',
          this.props.renderNode.node.uuid,
        );
        console.log('🔴 Node.render: translating:', this.props.translating);
        console.log(
