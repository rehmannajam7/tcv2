...
export interface Assets {
  ...
  // optional error message for loading/fetching issues
  error?: string;
}
...
export const updateAssets = (assets: AssetStore): UpdateAssetsAction => {
  ...
  if (store) {
    ...
    if ((assets as any)['captainKnowledgeBases']?.items) {
      store.setKeyedAssets(
        'captainKnowledgeBases',
        Object.keys((assets as any)['captainKnowledgeBases'].items),
      );
    }
  }
