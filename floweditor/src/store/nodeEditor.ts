...
export const settings = (
  ...
  console.log('🔵 nodeEditor reducer: settings function called');
  console.log('🔵 nodeEditor reducer: action type:', action.type);
  console.log(
    '🔵 nodeEditor reducer: Constants.UPDATE_NODE_EDITOR_SETTINGS:',
    Constants.UPDATE_NODE_EDITOR_SETTINGS,
  );
  console.log(
    '🔵 nodeEditor reducer: action type matches:',
    action.type === Constants.UPDATE_NODE_EDITOR_SETTINGS,
  );

  // Only process actions that are specifically for the nodeEditor
  if (action.type !== Constants.UPDATE_NODE_EDITOR_SETTINGS) {
    console.log(
      '🔵 nodeEditor reducer: ignoring non-nodeEditor action, returning current state:',
      state,
    );
    return state;
  }
  ...
  switch (action.type) {
    case Constants.UPDATE_NODE_EDITOR_SETTINGS:
      console.log(
        '🔵 nodeEditor reducer: UPDATE_NODE_EDITOR_SETTINGS action received',
      );
      console.log('🔵 nodeEditor reducer: current state:', state);
      console.log('🔵 nodeEditor reducer: action payload:', action.payload);
      console.log(
        '🔵 nodeEditor reducer: new settings:',
        action.payload!.settings,
      );
      const newSettings = action.payload!.settings;
      console.log(
        '🔵 nodeEditor reducer: returning new settings:',
        newSettings,
      );
      return newSettings;
    default:
      console.log(
        '🔵 nodeEditor reducer: default case, returning current state:',
        state,
      );
      return state;
  }
