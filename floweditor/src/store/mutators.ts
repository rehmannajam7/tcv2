...
export const addRevision = (
  ...
  // Ensure revisions exists and has items property
  if (!assets.revisions || !assets.revisions.items) {
    return assets;
  }
...
export const addLanguage = (languages: Asset[], language: Asset): Asset[] => {
  return mutate(languages, push([language])) as Asset[];
};
...
export const pruneDefinition = (definition: FlowDefinition): FlowDefinition => {
  const result = mutate(definition, {
    nodes: { $set: [] },
    _ui: { $merge: { nodes: {} } },
  });
  return result;
