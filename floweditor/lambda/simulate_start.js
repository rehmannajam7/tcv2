...
function pickExit(run, node, msg) {
  if (!node.router) return (node.exits || [])[0];               // no router → first exit
  if (node.router.wait && node.router.wait.type === 'msg') {    // wait for text → pick category by text
...
function runFlow(def, contact, resumeMsg) {
  ...
  let current = def.nodes.find(n => n.uuid === def.nodes[0].uuid); // start at first node
  const events = [];
  ...
  while (current) {
    ...
    const exit = pickExit(run, current, resumeMsg);
...
exports.handler = async (event, context, callback) => {
  try {
    ...
    const { flow, contact } = payload;
    if (!flow || !contact) throw new Error('Missing flow or contact');

    const { session, events } = runFlow(flow, contact, null);
