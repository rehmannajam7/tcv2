...
function executeActions(def, run, node, events) {
  (node.actions || []).forEach(act => {
    if (act.type === 'send_msg') {
      // Evaluate expressions in message text
      const evaluatedText = evaluateExpression(act.text, {
        contact: run.contact,
        flow: { results: run.results, name: def.name, uuid: def.uuid },
        step: run.path[run.path.length - 1] || {},
        channel: {}, // Add channel data if available
        date: {}
      });
    ...
    if (act.type === 'set_run_result') {
      ...
      // Evaluate expressions in result value
      const evaluatedValue = evaluateExpression(act.value, {
        contact: run.contact,
        flow: { results: run.results, name: def.name, uuid: def.uuid },
        step: run.path[run.path.length - 1] || {},
        channel: {},
      ...
      run.results[act.name] = { 
        value: evaluatedValue, 
        category: act.category || evaluatedValue, 
...
function pickExit(run, node, msg) {
  ...
  // Handle expression-based routing
  if (node.router.operand) {
    const context = {
      contact: run.contact,
      flow: { results: run.results, name: run.flow_uuid, uuid: run.flow_uuid },
      step: run.path[run.path.length - 1] || {},
      channel: {},
      date: {}
    };
    
    const operandValue = evaluateExpression(node.router.operand, context);
    
    // Find matching category based on operand value
    const matchingCategory = (node.router.categories || []).find(cat => {
      if (cat.test) {
        // Evaluate the test expression
        const testValue = evaluateExpression(cat.test, context);
        return testValue === operandValue;
      }
      return cat.name === operandValue;
    });
    
    return (node.exits || []).find(e => e.uuid === (matchingCategory ? matchingCategory.exit_uuid : null)) || (node.exits || [])[0];
  }
