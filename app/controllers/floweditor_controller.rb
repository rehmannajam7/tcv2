class FloweditorController < ApplicationController
  ...
  FLOWEDITOR_BUILD_DIR = Rails.root.join('floweditor/build')

  def index
    # Serve the main FlowEditor index.html file
    render file: FLOWEDITOR_BUILD_DIR.join('index.html'),
           layout: false,
  ...
  def assets
    ...
    full_path = FLOWEDITOR_BUILD_DIR.join(file_path)

    # Security check - ensure the path is within the floweditor directory
    return head :not_found unless full_path.to_s.start_with?(FLOWEDITOR_BUILD_DIR.to_s)

    # Check if file exists
    return head :not_found unless File.exist?(full_path) && !File.directory?(full_path)
