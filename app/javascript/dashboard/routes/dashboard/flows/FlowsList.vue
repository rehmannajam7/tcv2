...
export default {
  ...
  methods: {
    ...
    statusLabel(flow) {
      if (flow?.status) {
        const s = String(flow.status).toLowerCase();
        if (s === 'active') return 'Active';
        if (s === 'inactive') return 'Inactive';
        return 'Draft';
      }
      return flow?.active ? 'Active' : 'Inactive';
    },
    statusColor(flow) {
      if (flow?.status) {
        const s = String(flow.status).toLowerCase();
      ...
      return flow?.active ? 'bg-green-500' : 'bg-gray-400';
    },
    ...
    editFlow(flowId) {
      // Open edit modal for existing flow if available, otherwise navigate
      const flow = this.flows.find(f => f.id === flowId);
      if (flow) {
        this.openEditModal(flow);
      } else {
        this.navigateToFlowEditor(flowId);
      }
    },
    openEditModal(flow) {
    ...
    onFlowSaved(updatedFlow) {
      const idx = this.flows.findIndex(f => f.id === updatedFlow.id);
      if (idx !== -1) {
        // Merge to retain other fields if server returned partial
        const merged = { ...this.flows[idx], ...updatedFlow };
        this.flows.splice(idx, 1, merged);
      }
      this.closeEditModal();
    },
  ...
  >
    ...
    <div v-if="isLoading" class="flex items-center justify-center py-12">
      <div class="text-center">
        <div
          class="animate-spin rounded-full h-8 w-8 border-b-2 border-woot-500 mx-auto mb-4"
        />
    ...
    >
      <div class="text-center">
        ...
        <h3 class="text-lg font-medium text-n-slate-12 mb-2">
          {{ $t('FLOWS.EMPTY_STATE.TITLE') }}
        </h3>
        <p class="text-n-slate-11 mb-4">
          {{ $t('FLOWS.EMPTY_STATE.MESSAGE') }}
        </p>
        <button
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              <h3
                class="text-lg font-medium text-n-slate-12 group-hover:text-n-brand transition-colors"
              >
                {{ flow.name }}
              </h3>
              <p
          ...
          <!-- Flow Metadata -->
          <div
            class="flex items-center justify-between text-xs text-n-slate-10 mb-2"
          >
          ...
          <div class="flex items-center gap-2">
            <div 
              ...
              :class="{
                ...
                'bg-gray-100 text-gray-800': flow.status === 'inactive',
                'bg-yellow-100 text-yellow-800': flow.status === 'draft'
              }"
            >
              <div class="w-2 h-2 rounded-full mr-1.5" :class="[statusColor(flow)]" />
              {{ statusLabel(flow) }}
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 class="text-lg font-medium text-n-slate-12">
              {{ $t('FLOWS.DELETE.CONFIRM_TITLE') }}
            </h3>
            <p class="text-sm text-n-slate-11">
              {{ $t('FLOWS.DELETE.CONFIRM_MESSAGE') }}
            </p>
          </div>
        </div>
